...
public class MyHttpProxy extends ProxyConfiguration.Proxy
{
	public MyHttpProxy(String host, int port)
	{
		this(new Origin.Address(host, port), false);
	}

	public MyHttpProxy(Origin.Address address, boolean secure)
	{
		super(address, secure);
	}

	/*@Override*/
	public boolean isTunnel(Request request) {
	...
	static class ProxyConnection  extends AbstractConnection {

		protected ProxyConnection(EndPoint endp, Executor executor) {
			super(endp, executor);
		}
